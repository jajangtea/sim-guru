...
class CIdentitasPegawai extends MainPageM {
    ...
    public function onLoad($param) {
        parent::onLoad($param);
        $this->showIdentitasPegawai = true;
        $this->createObj('DMaster');
        if (!$this->IsPostBack && !$this->IsCallback) {
            if (!isset($_SESSION['currentPageIdentitasPegawai']) || $_SESSION['currentPageIdentitasPegawai']['page_name'] != 'sa.dmaster.IdentitasPegawai') {
                $_SESSION['currentPageIdentitasPegawai'] = array('page_name' => 'sa.dmaster.IdentitasPegawai', 'page_num' => 0, 'search' => false);
            }
            $_SESSION['currentPageIdentitasPegawai']['search'] = false;
            $this->populateData();
        }
    }
    ...
    public function Page_Changed($sender, $param) {
        $_SESSION['currentPageIdentitasPegawai']['page_num'] = $param->NewPageIndex;
        $this->populateData($_SESSION['currentPageIdentitasPegawai']['search']);
    }

    public function searchRecord($sender, $param) {
        $_SESSION['currentPageIdentitasPegawai']['search'] = true;
        $this->populateData($_SESSION['currentPageIdentitasPegawai']['search']);
    }
    ...
    protected function populateData($search = false) {
        if ($search) {
            ...
            $jumlah_baris = $this->DB->getCountRowsOfTable("pegawai", 'nip');
            $str = "SELECT * FROM pegawai";
        }
        $this->RepeaterS->CurrentPageIndex = $_SESSION['currentPageIdentitasPegawai']['page_num'];
        ...
        if ($limit < 0) {
            ...
            $_SESSION['currentPageIdentitasPegawai']['page_num'] = 0;
        }
    ...
    public function saveData($sender, $param) {
        if ($this->Page->isValid) {
            ...
            $kode_sekolah = addslashes($this->cmbAddIdentitasPegawai->Text);
    ...
    public function editRecord($sender, $param) {
        ...
        $this->cmbEditIdentitasPegawai->Text = $result['kode_sekolah'];
...
//        $this->cmbEditStatus->Text = $result['status'];
    ...
    public function updateData($sender, $param) {
        if ($this->Page->isValid) {
            ...
            $kode_sekolah = addslashes($this->cmbEditIdentitasPegawai->Text);
